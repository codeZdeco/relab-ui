...
function TableBody(props: TableBodyProps) {
  ...
  const { values, fields, selected, conditions, BodyProps, RowProps } =
    TableProps;

  /** Extract primary field */
  const primaryField = fields.filter((field: FieldProps) => field.isPrimary)[0];

  const isSelected = (value: { [key: string]: any }): boolean => {
    return (selected ? selected : []).includes(value[primaryField.id]);
  };

  const consumeRowProps = (value: { [key: string]: any }) => {
    ...
    conditions &&
      conditions.map(
        (
          condition: (
            value: any,
          ) => React.BaseHTMLAttributes<HTMLTableRowElement>,
        ) => {
  ...
  const renderRow = (value: { [key: string]: any }) => {
  ...
  const sortedValues = useMemo(
    () => stableSort(values, getComparator(orderDirection, orderBy)),
    [orderBy, orderDirection, values],
  );

  return (
    <MuiTableBody {...BodyProps}>{sortedValues.map(renderRow)}</MuiTableBody>
  );
}
